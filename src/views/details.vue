...
<script>
import { detailsNotice } from "../services/list";
export default {
  data() {
    return {
      obj: {
  ...
  mounted() {
    detailsNotice(this.obj).then(item => {
      var datas = item.data[0];
      var content = datas.content;
      this.list = content;
      this.title = datas.title;
      this.date = datas.addTime;
    });
    var item = $(".fontSi");
    item.on("click", "span", function(event) {
      var data = $(event.target)[0].innerHTML;
      var span = $(".mylist")
        .children("p")
        .children("span");
      if (data === "小") {
